...
import com.jihi.contact_project_v5.authentication.session.AuthenticationService;
import com.jihi.contact_project_v5.authentication.session.SessionManager;
import com.jihi.contact_project_v5.model.contact.ContactDTO;
import com.jihi.contact_project_v5.model.contact.ContactEntity;
import com.jihi.contact_project_v5.model.repository.ContactRepository;
...
public class ContactController {
    ...
    public String addContact(RedirectAttributes attributes, ContactDTO dto) {
        ...
        switch (dto.getRelationship()) {
            ...
            default:
                ...
                attributes.addFlashAttribute("msg", "관계는 가족, 친구, 기타 중 하나여야합니다");
    ...
    public String viewContacts(Model model) {
        System.out.println("oaisdjfoasjfoaisjfoasidfjasdjfoasdjfoi");
        for (ContactEntity contact:  this.contactRepository.getAllContacts(this.sessionManager.getCurrentMemberId())) {
    ...
    public String editContact(RedirectAttributes attributes, @PathVariable int id, ContactDTO dto) {
        ...
        // 아닐경우 메세지 출력/contacts 로 redirect
        attributes.addFlashAttribute("msg", "고객님의 연락처만 보세요");
        return "redirect:/contacts";
    }
    ...
    public String deleteContact(RedirectAttributes attributes, @PathVariable int id) {
        ...
        if (this.contactRepository.getContact(memberId, id) != null) {
            ...
            // 아닐경우 메시지 출력 /contacts 로 redirect
            attributes.addFlashAttribute("msg", "고객님의 연락처만 보세요");
        }
        return "redirect:/contacts";
    }
